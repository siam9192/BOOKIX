...
const applyCoupon = async (
  { code, items }: { code: string; items: TCart[] },
  userId: string,
) => {
  ...
  const books = await Book.find({
    _id: { $in: items.map((item) => objectId(item.book.toString())) },
  });
  ...
  if (coupon.applicable_categories !== '**') {
    books.forEach((item) => {
      if (!coupon.applicable_categories.includes(item.category)) {
        throw new AppError(
          httpStatus.NOT_ACCEPTABLE,
          `This can is applicable for ${(coupon.applicable_categories as string[]).join(',')} category books`,
        );
      }
    });
  }
