...
const googleCallback = async(payload:TUser)=>{
 const user = await User.findOne({email:payload.email,registered_by:TRegistrationOption.GOOGLE_AUTH})
 const tokenPayload = {
  id: user?._id,
  role: user?.role,
}
...
if(!user){
 const user = await UserService.createUserIntoDB(payload,TRegistrationOption.GOOGLE_AUTH)
  if(!user){
