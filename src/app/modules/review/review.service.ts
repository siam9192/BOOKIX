...
const createReviewIntoDB = async (userId:string,payload: TReview) => {
  ...
  const isReviewed = order.books.find(item=>item.book.toString() === payload.book.toString())?.is_reviewed
 ...
 try {
  const updateResult = await Order.updateOne({_id:order._id,'books.book':objectId(payload.book.toString())},{'books.$.is_reviewed':true},{session})
...
const getBookReviewsFromDB = async (bookId: string, query: any) => {
  ...
  const result = await new QueryBuilder(Review.find(), query).find().paginate().populate('customer').get();
  const meta =  await new QueryBuilder(Review.find(), query).find().paginate().populate('customer').getMeta();
  return {
    result,
    meta,
  };
