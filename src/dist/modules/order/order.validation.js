...
const paymentMethodUnion = zod_1.z.union(
  Object.values(payment_interface_1.TPaymentMethod).map((item) =>
    zod_1.z.literal(item),
  ),
);
...
const createOrderValidation = zod_1.z.object({
  items: zod_1.z
    .array(itemSchema)
    .nonempty('At least one book must be provided'),
