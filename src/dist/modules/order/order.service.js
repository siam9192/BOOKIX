...
const createOrderIntoDB = (res, userId, payload) =>
  __awaiter(void 0, void 0, void 0, function* () {
    const bookObjectIds = payload.items.map((book) =>
      (0, func_1.objectId)(book.bookId),
    );
    ...
    if (payload.coupon) {
      ...
      if (
        coupon.discount_type === coupon_interface_1.TCouponDiscountType.FIXED
      ) {
    ...
    const paymentData = {
      transaction_id:
        'PP' + crypto_1.default.randomBytes(8).toString('hex').toUpperCase(),
...
const managePaymentSuccessOrdersIntoDB = (paymentId) =>
  __awaiter(void 0, void 0, void 0, function* () {
    ...
    const updatePayment = yield payment_model_1.Payment.updateOne(
      { _id: payment._id },
      { success: true },
    );
    ...
    const orderedBooks = yield order_model_1.Order.findOneAndUpdate(
      { payment: (0, func_1.objectId)(paymentId) },
      { is_paid: true },
      { new: true },
    ).select('books');
    ...
    if (orderedBooks) {
      Promise.all(
        orderedBooks.items.map((item) =>
          book_model_1.Book.findByIdAndUpdate(item.book, {
...
const managePaypalPaymentSuccessOrdersIntoDB = (res, query) =>
  __awaiter(void 0, void 0, void 0, function* () {
    const manageOrder = (saleId) =>
      __awaiter(void 0, void 0, void 0, function* () {
        const payment = yield payment_model_1.Payment.findById(
          query.orderPaymentId,
        );
        //  Checking payment existence
        if (!payment) {
          throw new AppError_1.default(
        ...
        try {
          ...
          // Updating order paid status
          const orderedBooks = yield order_model_1.Order.findOneAndUpdate(
            { payment: (0, func_1.objectId)(query.orderPaymentId) },
            { is_paid: true },
            { new: true, session },
          ).select('items');
          // Updating books available_stock
          if (orderedBooks) {
            yield Promise.all(
              orderedBooks.items.map((item) =>
                book_model_1.Book.findByIdAndUpdate(
                  item.book,
                  { $inc: { available_stock: -item.quantity } },
                  { session },
                ),
          ...
          paypal_1.Paypal.refund(
            saleId,
            payment.amount.subtotal,
            config_1.default.order_cancel_url,
            res,
          );
        }
      });
...
const managePaymentCanceledOrderIntoDB = (paymentId) =>
  __awaiter(void 0, void 0, void 0, function* () {
...
const getCurrentUserOrdersFromDB = (userId, query) =>
  __awaiter(void 0, void 0, void 0, function* () {
    const stages = [
      {
        $lookup: {
...
const updateOrderStatus = (payload) =>
  __awaiter(void 0, void 0, void 0, function* () {
    const order = yield order_model_1.Order.findById(payload.orderId).populate(
      'payment',
    );
    ...
    if (
      orderStatus === order_interface_1.TOrderStatus.PENDING &&
      ...
      orderStatus === order_interface_1.TOrderStatus.PROCESSING &&
      ...
      orderStatus === order_interface_1.TOrderStatus.IN_TRANSIT &&
    ...
    const result = yield order_model_1.Order.findByIdAndUpdate(
      payload.orderId,
...
const cancelOrderIntoDB = (userId, userRole, orderId) =>
  __awaiter(void 0, void 0, void 0, function* () {
    ...
    const updateStatus = yield order_model_1.Order.updateOne(
      { _id: (0, func_1.objectId)(orderId) },
      { status: order_interface_1.TOrderStatus.CANCELLED },
    );
...
const getCustomerYetToReviewOrdersFromDB = (userId) =>
  __awaiter(void 0, void 0, void 0, function* () {
    const result = yield order_model_1.Order.aggregate([
      {
        $match: {
...
const getOrderDetailsFromDB = (orderId) =>
  __awaiter(void 0, void 0, void 0, function* () {
    const result = order_model_1.Order.findById(orderId).populate('payment');
    return result;
  });
const getUserOrderHistoryFromDB = (userId, query) =>
  __awaiter(void 0, void 0, void 0, function* () {
    const stages = [
      {
        $lookup: {
