...
const initiateAccountCreation = (payload) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const result = yield accountCreation_request_model_1.AccountCreationRequest.create(payload);
    ...
    const token = (0, func_1.generateJwtToken)(tokenPayload, config_1.default.jwt_ac_verify_secret, '10m');
    return {
        secret: token,
    };
});
const verifyOtpFromDB = (payload) => __awaiter(void 0, void 0, void 0, function* () {
    // Decoding jwt token
    const decode = (0, func_1.verifyToken)(payload.secret, config_1.default.jwt_ac_verify_secret);
    if (!decode) {
        throw new AppError_1.default(http_status_1.default.NOT_ACCEPTABLE, 'Invalid request');
    }
    // Finding data
    const data = yield accountCreation_request_model_1.AccountCreationRequest.findOne({
    ...
    const createdUser = yield user_service_1.UserService.createUserIntoDB(userData, user_interface_1.TRegistrationOption.EMAIL);
    // Deleting account request data
    yield accountCreation_request_model_1.AccountCreationRequest.deleteOne({ _id: new mongoose_1.Types.ObjectId(data._id), email: data.email }, { session });
...
const resendOtp = (payload) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const accountCreationRequest = yield accountCreation_request_model_1.AccountCreationRequest.findOne({
        _id: (0, func_1.objectId)(decode.id),
        // email: decode.email,
    });
    // Checking request existence
    if (!accountCreationRequest) {
        throw new AppError_1.default(http_status_1.default.NOT_FOUND, 'Time expired');
    }
    // Difference between request time and previous otp send time
    const difference = (new Date(payload.requestTime || new Date()).valueOf() -
        new Date(accountCreationRequest.updatedAt).valueOf()) /
        1000;
    // If difference less  than 120 seconds  than  resend otp can not be possible
    if (difference < 120) {
        throw new AppError_1.default(http_status_1.default.NOT_ACCEPTABLE, 'Can not be able to resend otp before 2 minutes ');
    }
    ...
    // Updating otp
    const updatedAccountCreationRequest = yield accountCreation_request_model_1.AccountCreationRequest.findOneAndUpdate({ _id: new mongoose_1.Types.ObjectId(decode.id), email: decode.email }, { otp: hashedOtp }, { new: true, runValidators: true });
    // Checking is otp updated successfully
    if (!updatedAccountCreationRequest) {
        throw new AppError_1.default(400, 'Something went wrong please try again');
    }
    yield (0, sendEmail_1.sendEmailVerificationMail)(updatedAccountCreationRequest.email, newOtp);
    return true;
});
