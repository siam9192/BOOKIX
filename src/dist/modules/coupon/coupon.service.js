...
const createCouponIntoDB = (payload) => __awaiter(void 0, void 0, void 0, function* () {
    const coupon = yield coupon_model_1.default.findOne({ coupon_code: payload.coupon_code });
...
const getCouponByCodeFromDB = (code) => __awaiter(void 0, void 0, void 0, function* () {
    return yield coupon_model_1.default.findOne({
        coupon_code: code,
        valid_until: { $gte: new Date() },
    });
});
const applyCoupon = (_a, userId_1) => __awaiter(void 0, [_a, userId_1], void 0, function* ({ code, items }, userId) {
    const coupon = yield coupon_model_1.default.findOne({ coupon_code: code });
    ...
    if (coupon.specific_customers !== '**') {
        ...
        if (!isCustomerExist) {
            throw new AppError_1.default(http_status_1.default.NOT_ACCEPTABLE, 'This Coupon is not applicable for you account');
        }
    }
    ...
    if (validFrom > currentDate) {
        throw new AppError_1.default(http_status_1.default.NOT_ACCEPTABLE, 'This coupon can not be applicable this time');
    }
    if (currentDate > expireDate) {
        throw new AppError_1.default(http_status_1.default.NOT_ACCEPTABLE, 'This coupon is expired');
    }
    const books = yield book_model_1.Book.find({
        _id: { $in: items.map((item) => (0, func_1.objectId)(item.book.toString())) },
    });
    ...
    if (coupon.applicable_categories !== '**') {
        books.forEach((item) => {
            if (!coupon.applicable_categories.includes(item.category)) {
                throw new AppError_1.default(http_status_1.default.NOT_ACCEPTABLE, `This can is applicable for ${coupon.applicable_categories.join(',')} category books`);
            }
        });
    }
    let subtotal = 0;
    ...
    if (subtotal < coupon.minimum_purchase_amount) {
        throw new AppError_1.default(http_status_1.default.NOT_ACCEPTABLE, `This coupon is  applicable on at least ${coupon.minimum_purchase_amount} amount purchase `);
    }
    if (coupon.discount_type === coupon_interface_1.TCouponDiscountType.PERCENTAGE) {
...
const getCouponsFromDB = (query) => __awaiter(void 0, void 0, void 0, function* () {
    return yield coupon_model_1.default.find();
});
const updateCouponIntoDB = (couponId, payload) => __awaiter(void 0, void 0, void 0, function* () {
    const coupon = yield coupon_model_1.default.findById(couponId);
    // Checking coupon existence
    if (!coupon) {
        throw new AppError_1.default(http_status_1.default.NOT_FOUND, 'Coupon not found');
    }
    return yield coupon_model_1.default.findByIdAndUpdate(couponId, payload, { new: true });
});
const deleteCouponFromDB = (couponId) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    return yield coupon_model_1.default.findByIdAndDelete(couponId, { new: true });
});
